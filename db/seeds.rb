# This file should contain all the record creation needed to seed the database with its default values.
# The data can then be loaded with the rails db:seed command (or created alongside the database with db:setup).
#
# Examples:
#
#   movies = Movie.create([{ name: 'Star Wars' }, { name: 'Lord of the Rings' }])
#   Character.create(name: 'Luke', movie: movies.first)

require 'open-uri'

User.destroy_all
Video.destroy_all
Comment.destroy_all
Like.destroy_all
Subscribe.destroy_all
ActiveRecord::Base.connection.tables.each do |t|
  ActiveRecord::Base.connection.reset_pk_sequence!(t)
end

# Users
User.create(username: 'demo', password: 'password', email:'jane@demoemail.com', first_name: 'Jane', last_name: 'McDemo', profile_image_url: 'https://i.ibb.co/x8170Cw/1516927665677-e-2159024400-v-beta-t-Rx-N5n-z8rd8da6-Wvi-LYt-F5-Ke-Tvv-Jib-FL1sw-NI45-QTo-U.jpg')
User.create(username: 'HollyJPhilly', password: 'password', email:'hello.hollyphillips@gmail.com', first_name: 'Holly', last_name: 'Phillips', profile_image_url: 'https://avatars.githubusercontent.com/u/61254925?v=4')
User.create(username: 'Zoom Ninja', password: 'password', email:'contact@hollyjphillips.com', first_name: 'Holly', last_name: 'Phillips', profile_image_url: 'https://i.ibb.co/dGQnKTB/ZoomUser.png')
User.create(username: 'banana', password: 'password', email:'banana@peel.com', first_name: 'bana', last_name: 'na', profile_image_url: 'https://i.ibb.co/XDTygG1/image.png')
User.create(username: 'dinosaur', password: 'asteroid', email:'tee@rex.com', first_name: 'dino', last_name: 'saurus')

# Videos
vid2 = Video.create!(title: "How I Got Into Software Engineering (in 60 seconds)", description: "Transcription:\nWhen the pandemic hit in March 2020, I was working for Dida Academy, a progressive learning center in Brooklyn, NY. The onset of quarantines meant my students and I had to transition to a remote classroom almost overnight. Being someone who loves to play with new tech, I used Glide ,a platform I had been teaching myself at the time, to build my students a web app that could serve all the functionality needed for an online classroom. I built the app in less than a week and then onboarded parents, students, and staff who continued to use it into this school year. Even after the app was deployed, I found myself still thinking about features I could add and googling all sorts of questions about APIs and data structures. At that point I began thinking, maybe this is something I should go into? And that’s how my journey into Software Engineering began. Since building the app, I’ve learned more about data structures, algorithms, multiple programming languages, and am fluent in more web stacks than I would have thought possible a year or two ago. I’m super excited to make an impact just like I did with the Dida Academy app as I continue my career in Software Engineering.", uploader_id: 2)
file = open("https://yourtube-seeds.s3.amazonaws.com/dida.mp4")
vid2.moviefile.attach(io: file, filename: "dida.mp4")
file = open("https://yourtube-seeds.s3.amazonaws.com/didathumb.png")
vid2.thumbnail.attach(io: file, filename: "didathumb.png")

vid5 = Video.create!(title: "My Top 5 Zoom Filters", description: "Top 5 Zoom Filters (1).Div Life (2).Wonder Woman (3).Skynet (4).Magical Forest (5).YourTube Swag", uploader_id: 3)
file = open("https://drive.google.com/uc?export=download&id=1ap6WFKEibYuW3KjJ8JT3lSDseONXuBDi")
vid5.moviefile.attach(io: file, filename: "topText.mp4")
file = open("https://yourtube-seeds.s3.amazonaws.com/topthumb.png")
vid5.thumbnail.attach(io: file, filename: "topthumb.png")

vid1 = Video.create!(title: "Hype Tour - MERN Stack Project", 
    description: "HYPE is a social networking app for adults who haven't outgrown schoolyard games.", 
    uploader_id: 2)
file = open("https://yourtube-seeds.s3.amazonaws.com/hype.mp4")
vid1.moviefile.attach(io: file, filename: "hype.mp4")
file = open("https://yourtube-seeds.s3.amazonaws.com/hypethumb.png")
vid1.thumbnail.attach(io: file, filename: "hypethumb.png")

vid6 = Video.create!(title: "FUNNY Filters for ZOOM", description: "My Top 3 Funny Filters (1).Le Mustache (2).Distracted Boyfriend Meme (3).BANANA", uploader_id: 3)
file = open("https://drive.google.com/uc?export=download&id=1wZh6cSWbB8uXscC4NfpD0cK-KCmjW7WG")
vid6.moviefile.attach(io: file, filename: "funny.mp4")
file = open("https://drive.google.com/uc?export=download&id=1Qg4BCHAvvSL-YWHl9LL8fImDvYB3f62y")
vid6.thumbnail.attach(io: file, filename: "funnythumb.png")

vid3 = Video.create!(title: "Pokesweeper Official Trailer", description: "A Pokemon-themed minesweeper game. Play at: https://hollyjphilly.github.io/Pokesweeper/", uploader_id: 2)
file = open("https://yourtube-seeds.s3.amazonaws.com/pokesweeper.mp4")
vid3.moviefile.attach(io: file, filename: "pokesweeper.mp4")
file = open("https://yourtube-seeds.s3.amazonaws.com/pokethumb.png")
vid3.thumbnail.attach(io: file, filename: "pokethumb.png")

vid7 = Video.create!(title: "Zoom Filters for Software Engineers", description: "Top 3 Zoom Filters for Work (1).Bug Hunting (2).Stylin' with Sass (3).Actually 'The Office'", uploader_id: 3)
file = open("https://drive.google.com/uc?export=download&id=1jMiaL9y_VyLxTpx2a9fJUrpo_Pk65zQY")
vid7.moviefile.attach(io: file, filename: "office.mp4")
file = open("https://drive.google.com/uc?export=download&id=1TXVAEuSSMXy827OU3HL30TGeq1wqBR19")
vid7.thumbnail.attach(io: file, filename: "officethumb.png")

vid4 = Video.create!(title: "RelaTable Demo", description: "RelaTable is a SQL JOIN visualizer I made after learning about relational databases. Check it out at: https://hollyjphilly.github.io/RelaTable/", uploader_id: 2)
file = open("https://yourtube-seeds.s3.amazonaws.com/relatable.mp4")
vid4.moviefile.attach(io: file, filename: "relatable.mp4")
file = open("https://yourtube-seeds.s3.amazonaws.com/relatablethumb.png")
vid4.thumbnail.attach(io: file, filename: "relatablethumb.png")

vid8 = Video.create!(title: "The Big Screen on Zoom", description: "Top 3 Zoom Filters from Movies (1).The Mandalorian (2).Harry Potter (3).The Matrix", uploader_id: 3)
file = open("https://drive.google.com/uc?export=download&id=16VwjNl9RZqh6M-rtqJpiMLzaY2cJXUXL")
vid8.moviefile.attach(io: file, filename: "movies.mp4")
file = open("https://drive.google.com/uc?export=download&id=1HR-W-KgedlleJMWgTD7JMhU65AXK57ll")
vid8.thumbnail.attach(io: file, filename: "moviethumb.png")

# 1. Demo
# 2. HollyJPhilly
# 3. Zoom Ninja
# 4. Banana
# 5. Dinosaur

# 1. How I Got Into Software Engineering (in 60 seconds) - 2 HollyJPhilly
# 2. My Top 5 Zoom Filters                              - 3 Zoom Ninja
# 3. Hype Tour - MERN Stack Project                     - 2 HollyJPhilly
# 4. FUNNY Filters for ZOOM                             - 3 Zoom Ninja
# 5. Pokesweeper Official Trailer                       - 2 HollyJPhilly
# 6. Zoom Filters for Software Engineers                - 3 Zoom Ninja
# 7. RelaTable Demo                                     - 2 HollyJPhilly
# 8. The Big Screen on Zoom                             - 3 Zoom Ninja

# Comments
com1 = Comment.create!(body: "Thanks for sharing your story!", commenter_id: 4, video_id: 1)
com2 = Comment.create!(body: "Wow, sounds like a great start.", commenter_id: 4, video_id: 1)
Comment.create!(body: "omg, the YourTube one - HA", commenter_id: 4, video_id: 2)
Comment.create!(body: "Never would have saw the second one coming.", commenter_id: 1, video_id: 2)
Comment.create!(body: "The README was a good read. Nice location autocomplete.", commenter_id: 1, video_id: 3)
Comment.create!(body: "Can't wait to make my avatar!", commenter_id: 5, video_id: 3)
Comment.create!(body: "Check em out!", commenter_id: 2, video_id: 4)
Comment.create!(body: "BAHAHAHAHAHAHA", commenter_id: 4, video_id: 4)
com3 = Comment.create!(body: "This trailer is so over the top for minesweeper.", commenter_id: 4, video_id: 5)
Comment.create!(body: "Hahaha I agree", commenter_id: 5, video_id: 5)
Comment.create!(body: "Stinks the office isn't on Netflix anymore", commenter_id: 5, video_id: 6)
Comment.create!(body: "LOLLLL bug huntingggg", commenter_id: 2, video_id: 6)
Comment.create!(body: "NoSQL is the future tho man", commenter_id: 4, video_id: 7)
Comment.create!(body: "The mandalorian was so good", commenter_id: 2, video_id: 7)
Comment.create!(body: "definitely wish Gorgu was real", commenter_id: 3, video_id: 7)


#Likes
com3.likes << Like.new(liker_id: 1, kind: "like")
com3.likes << Like.new(liker_id: 2, kind: "dislike")
vid1.likes << Like.new(liker_id: 1, kind: "like")
vid2.likes << Like.new(liker_id: 1, kind: "like")
vid3.likes << Like.new(liker_id: 1, kind: "like")
vid4.likes << Like.new(liker_id: 1, kind: "like")
vid5.likes << Like.new(liker_id: 1, kind: "like")
vid6.likes << Like.new(liker_id: 1, kind: "like")
vid7.likes << Like.new(liker_id: 1, kind: "like")
vid1.likes << Like.new(liker_id: 2, kind: "like")
vid2.likes << Like.new(liker_id: 2, kind: "like")
vid3.likes << Like.new(liker_id: 2, kind: "like")
vid4.likes << Like.new(liker_id: 2, kind: "like")
vid5.likes << Like.new(liker_id: 2, kind: "like")
vid8.likes << Like.new(liker_id: 2, kind: "like")
vid7.likes << Like.new(liker_id: 2, kind: "like")
vid1.likes << Like.new(liker_id: 3, kind: "like")
vid2.likes << Like.new(liker_id: 3, kind: "like")
vid3.likes << Like.new(liker_id: 3, kind: "like")
vid8.likes << Like.new(liker_id: 3, kind: "like")
vid5.likes << Like.new(liker_id: 3, kind: "like")
vid6.likes << Like.new(liker_id: 3, kind: "like")
vid7.likes << Like.new(liker_id: 3, kind: "like")
vid1.likes << Like.new(liker_id: 4, kind: "like")
vid2.likes << Like.new(liker_id: 4, kind: "like")
vid3.likes << Like.new(liker_id: 4, kind: "like")
vid8.likes << Like.new(liker_id: 4, kind: "like")
vid5.likes << Like.new(liker_id: 4, kind: "like")
vid6.likes << Like.new(liker_id: 4, kind: "like")
vid7.likes << Like.new(liker_id: 4, kind: "like")
vid1.likes << Like.new(liker_id: 5, kind: "like")
vid8.likes << Like.new(liker_id: 5, kind: "like")
vid3.likes << Like.new(liker_id: 5, kind: "like")
vid4.likes << Like.new(liker_id: 5, kind: "like")
vid5.likes << Like.new(liker_id: 5, kind: "like")
vid6.likes << Like.new(liker_id: 5, kind: "like")
vid7.likes << Like.new(liker_id: 5, kind: "like")
vid8.likes << Like.new(liker_id: 1, kind: "dislike")

Subscribe.create!(subscriber_id: 2, subscribee_id: 1)
Subscribe.create!(subscriber_id: 3, subscribee_id: 1)
Subscribe.create!(subscriber_id: 4, subscribee_id: 1)
Subscribe.create!(subscriber_id: 5, subscribee_id: 1)

Subscribe.create!(subscriber_id: 1, subscribee_id: 2)
Subscribe.create!(subscriber_id: 3, subscribee_id: 2)
Subscribe.create!(subscriber_id: 4, subscribee_id: 2)
Subscribe.create!(subscriber_id: 5, subscribee_id: 2)

Subscribe.create!(subscriber_id: 2, subscribee_id: 3)
Subscribe.create!(subscriber_id: 1, subscribee_id: 3)
Subscribe.create!(subscriber_id: 4, subscribee_id: 3)
Subscribe.create!(subscriber_id: 5, subscribee_id: 3)

Subscribe.create!(subscriber_id: 1, subscribee_id: 4)
Subscribe.create!(subscriber_id: 3, subscribee_id: 4)

Subscribe.create!(subscriber_id: 1, subscribee_id: 5)
Subscribe.create!(subscriber_id: 3, subscribee_id: 5)

View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)
View.create!(viewer_id: 1, video_id: 1)

View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)
View.create!(viewer_id: 1, video_id: 2)

View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)
View.create!(viewer_id: 1, video_id: 3)

View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)
View.create!(viewer_id: 1, video_id: 4)

View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)
View.create!(viewer_id: 1, video_id: 5)

View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)
View.create!(viewer_id: 1, video_id: 6)